...
#API key connection
def Api_connect():
    Api_Id="AIzaSyBub0bxtLv4A00EFe-GFiVXGWBoS-VZhOE"
...
client=pymongo.MongoClient("mongodb+srv://amsath:1234@cluster0.cqyulik.mongodb.net/?retryWrites=true&w=majority")
...
def channels_table(channel_name_s):
    ...
    for index,row in df_single_channel_detail.iterrows():
        ...
        try:
            ...
            show=f"provided channel name {channel_name_s} is already exists"
            return show
...
def tables(single_channel):
    show=channels_table(single_channel)
    if show:
        return show
    else:
...
with st.sidebar:
    ...
    st.caption("collecting the data from API")
    st.caption("Data Collection from youtube")
    st.caption("gethring data insert into MongoDB")
    st.caption(" Data migration MongoDB to SQL")
